classdef CartPole < handle
    ...
    properties
        ...
        total_mass = 1.1; % Total_mass = masspole + masscart;
        polelength = 0.5 % Half the pole's length        
        polemass_length = 0.05; % polemass_length = masspole * polelength;
        force_push_mag = 10.0; % Magnitude of the force_push
        tau = 0.02; % Time between state updates
        
        theta_threshold_radians = 12 * 2 * pi / 360; % Allowed theta range
        x_threshold = 2.4; % Allowed x range
    ...
    methods
        ...
        function [state_next, reward, done] = nextstate(obj, action)
            ...
            % -------------------------------------------------------------
            % Note to make the theta spans a similar range with other 
            % outputs, a constant factor 10 is applied. (This helps to make
            % the input voltage represeting theta to memristors larger,
            % thus accurate.)
            theta_x10 = theta * 10;
            % -------------------------------------------------------------
        ...
        function state_new = reset_episode(obj)
            ...
            % Initalization of a state (theta_x10 here)
            state_new=0.1*(rand(4,1)-0.5);
                      
            % Update current state and done_flag
            obj.state_now=state_new;
                        
            % Change the 'internal' state(3) back to theta (from theta_x10)
        ...
        function CartPole_plot(obj, plot_axis_obj, extrainfo)
            ...
            theta = obj.state_now(3);
            x_pole = [pole_height * sin(theta) - 0.5 * pole_width * cos(theta),...
                pole_height * sin(theta) + 0.5 * pole_width*cos(theta),...
                0.5 * pole_width * cos(theta), -0.5 * pole_width * cos(theta)];
            x_pole = x_pole + x;
            y_pole = [pole_height * cos(theta) + 0.5 * pole_width * sin(theta),...
                pole_height * cos(theta) - 0.5 * pole_width*sin(theta),...
