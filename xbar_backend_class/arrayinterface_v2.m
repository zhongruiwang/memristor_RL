...
% Interface between network objects & array objects
    ...
    properties
        ...
        % Blind memristor update parameters
        th_set = 0;
        th_reset = -0;
        ...
        % Delta_conductancd / Delta_V_gate
        ratio_G_Vg = 125e-6;  % (100e-6 for cartpole)
    ...
    methods
        ...
        function obj = arrayinterface_v2(base,net_size, weight_scaling)
            ...
            for i = 1:(length(net_size) - 1)
                ...
                if isequal(obj.base.subs{i}.net_size, [2*net_size(i), net_size(i + 1)])
                    ...
                    % Gate voltage to initialize random weight
        ...
        function initialize_weights(obj)
            ...
            % SET the array (base on the random weights)
            obj.base.update_subs(obj.V_set, 'GND', obj.V_gate_last);
        ...
        function dpe_result = dot_product(obj, layer, input, varargin)
            ...
            voltage_scaling(~any(input)) = 1; % if input is '000...000'
            ...
            if obj.diff_pair_ori(layer) == 1
				...
				if layer == 1 % Layer 1 (specific TIA gain)
	                I_output = obj.base.subs{layer}.read_current(V_input, 'gain', 2);
                else % Layer 2,3 (default TIA gain)
        ...
        function set_weights_gradient(obj, grad)
            ...
            % Initialize reset, gate, gate apply
            ...
            for layer = 1:length(grad)
                ...
                % Gradient to voltage conversion
